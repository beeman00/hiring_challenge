...
That user ID is used to retrieve the user's current friends list store in Redis. This key contains a serialized instance of `app\domain\chat\FriendsList`. That instance is then filled with presence status of the relevant users (also stored in Redis).
...
You shouldn't pay much attention to what `app\domain\chat\FriendsList` class does. It's just a data transport object. Those cache keys are generated by another processes.
...
- Microframeworks are welcome, but not mandatory.
